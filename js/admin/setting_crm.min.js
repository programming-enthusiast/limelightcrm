jQuery( document ).ready(function( $ ) 
{
    ...
    function show_alert(type, msg) 
    {
        ...
        else if (type == 'crm_password')
        {
            $('.crm_password_alert').html(msg);
            $('.crm_password_alert').fadeIn(1000, function () {
                $('.crm_password_alert').fadeOut(3000);
            });
        }
        else if (type == 'api_password')
        {
            $('.api_password_alert').html(msg);
            $('.api_password_alert').fadeIn(1000, function () {
                $('.api_password_alert').fadeOut(3000);
            });
        }
    }
    ...
    $('.modal_btn_crm_add').click(function ()
    {
        ...
        if ($('.add_crm_password').val() != $('.add_crm_repassword').val())
        {
        ...
        if ($('.add_api_password').val() != $('.add_api_repassword').val())
        {
    ...
    // edit crm password function
    $('.table_crm_body').on('click', '.setting_crm_password', function (e)
    {
        crm_id = $(this).prop('id');
    ...
    $('.modal_btn_crm_password').click(function ()
    {
        if ($('.edit_crm_password').val() == '')
        {
            show_alert('crm_password', 'Please input CRM Password.');
            $('.edit_crm_password').focus();
            return;
        }

        if ($('.edit_crm_password').val() != $('.edit_crm_repassword').val())
        {
            show_alert('crm_password', 'Doesn\'t match CRM Password.');
            $('.edit_crm_repassword').focus();
            return;
        }
        
        $('#crm_password_modal').modal('toggle');
        ajaxCrmPassword();
    });

    // edit api password function
    $('.table_crm_body').on('click', '.setting_api_password', function (e)
    {
        crm_id = $(this).prop('id');
    ...
    $('.modal_btn_api_password').click(function ()
    {
        if ($('.edit_api_password').val() == '')
        {
            show_alert('api_password', 'Please input API Password.');
            $('.edit_api_password').focus();
            return;
        }

        if ($('.edit_api_password').val() != $('.edit_api_repassword').val())
        {
            show_alert('api_password', 'Doesn\'t match API Password.');
            $('.edit_api_repassword').focus();
            return;
        }
        
        $('#api_password_modal').modal('toggle');
        ajaxApiPassword();
    });
                ...
                else
                {
                    ...
                    if (crm_count > 0)
                    {
                        for (var i = 0; i < crm_count; i ++)
                        {
                            ...
                            html += '<td id="goal_' + obj[i][0] + '">' + obj[i][7] + '</td>';
                            if (obj[i][8] == '1')
                            	html += '<td id="paused_' + obj[i][0] + '">Paused</td>';
                            else
                            	html += '<td id="paused_' + obj[i][0] + '"></td>';

                            if (obj[i][9] == '0000-00-00' || obj[i][9] == null)
                            {
                                html += '<td id="valid_' + obj[i][0] + '" style="background-color:#ffa5a5">No set password date</td>';
                            }
                            else
                            {
                                var first = obj[i][9].split('-');
                                var second = obj[i][10].split('-');

                                var firstDate = Date.UTC(first[0], first[1] - 1, first[2]);
                                var secondDate = Date.UTC(second[0], second[1] - 1, second[2]);
                                var vaild_days = 30 - Math.abs((firstDate.valueOf() - secondDate.valueOf())/(24*60*60*1000));
                                if (vaild_days < 0)
                                    vaild_days = 0;
                                
                                if (vaild_days < 5)
                                    html += '<td id="valid_' + obj[i][0] + '" style="background-color:#ffa5a5">' + (vaild_days) + '</td>';
                                else
                                    html += '<td id="valid_' + obj[i][0] + '">' + (vaild_days) + '</td>';
                            }
                            html += '<td>';
                            ...
                            html += '<button type="button" class="btn btn-link btn-sm setting_crm_password" id="' + obj[i][0] + '" data-toggle="modal" data-target="#crm_password_modal"><span class="glyphicon glyphicon-lock" aria-hidden="true"></span>&nbsp;CRM Password</button>';
                            html += '<button type="button" class="btn btn-link btn-sm setting_api_password" id="' + obj[i][0] + '" data-toggle="modal" data-target="#api_password_modal"><span class="glyphicon glyphicon-lock" aria-hidden="true"></span>&nbsp;API Password</button>';
    ...
    function ajaxCrmPassword()
    {
        ...
        {
            type : 'GET',
            url : '../daemon/ajax/setting_crm_crmpass.php',
            data : {
                'crm_id' : crm_id, 
                'crm_password' : $('.edit_crm_password').val()
            },
    ...
    function ajaxApiPassword()
    {
        ...
        {
            type : 'GET',
            url : '../daemon/ajax/setting_crm_apipass.php',
            data : {
                'crm_id' : crm_id, 
                'api_password' : $('.edit_api_password').val()
            },
